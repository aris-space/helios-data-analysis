    ...
    for id in config_ids:
        ...
        if config_opt == 5 or 1:
            ...
            if effic_opt == 1 or 2 or 3:
                ...
                yaxis8 = [
                    "Discharge coefficient * 1000 lsp[s]",
                ...
                sensor_cut0["time"] = pd.to_datetime(
                    sensor_cut0["timestamp"] + 3600 * 2, unit="s"
                )
                sensor_cut0["time_relative"] = sensor_cut0["timestamp"] - start8

                sensor_cut1 = values1[
                    (values1["timestamp"] >= start8) & (values1["timestamp"] <= end8)
                ]
                sensor_cut1["time"] = pd.to_datetime(
                    sensor_cut1["timestamp"] + 3600 * 2, unit="s"
                )
                sensor_cut1["time_relative"] = sensor_cut1["timestamp"] - start8

                sensor_cut2 = values2[
                    (values2["timestamp"] >= start8) & (values2["timestamp"] <= end8)
                ]
                sensor_cut2["time"] = pd.to_datetime(
                    sensor_cut2["timestamp"] + 3600 * 2, unit="s"
                )
                sensor_cut2["time_relative"] = sensor_cut2["timestamp"] - start8

                sensor_cut3 = values3[
                    (values3["timestamp"] >= start8) & (values3["timestamp"] <= end8)
                ]
                sensor_cut3["time"] = pd.to_datetime(
                    sensor_cut3["timestamp"] + 3600 * 2, unit="s"
                )
                sensor_cut3["time_relative"] = sensor_cut3["timestamp"] - start8

                sensor_cut4 = values4[
                    (values4["timestamp"] >= start8) & (values4["timestamp"] <= end8)
                ]
                sensor_cut4["time"] = pd.to_datetime(
                    sensor_cut4["timestamp"] + 3600 * 2, unit="s"
                )
                sensor_cut4["time_relative"] = sensor_cut4["timestamp"] - start8

                sensor_cut5 = values5[
                    (values5["timestamp"] >= start8) & (values5["timestamp"] <= end8)
                ]
                sensor_cut5["time"] = pd.to_datetime(
                    sensor_cut5["timestamp"] + 3600 * 2, unit="s"
                )
                    ...
                    fig8.add_trace(
                        go.Scatter(
                            ...
                            / (
                                ...
                                * 789
                                * (
                                    sensor_cut4["value"].rolling(30).mean()
                                    - sensor_cut1["value"].rolling(30).mean()
                                )
                            )
                            ** 0.5,
                            name="FI_cd_fss_L6",
                    ...
                    fig8.add_trace(
                        go.Scatter(
                            ...
                            / (
                                ...
                                * 1175
                                * (
                                    sensor_cut5["value"].rolling(30).mean()
                                    - sensor_cut1["value"].rolling(30).mean()
                                )
                            )
                            ** 0.5,
                ...
                if effic_opt == 1:
                    fig8.update_layout(
                        annotations=[
                            {**element[0], **{"y": element[1]}} for element in y
                        ],
                ...
                fig8.show()
                if effic_opt == 1:
                    filepath = f"{GLOBAL_PLOT_FILE_PREFIX}{date_string8}_{id}_efficiency-L6.html"
                    i = 0
                    while os.path.exists(filepath):
                        filepath = f"{GLOBAL_PLOT_FILE_PREFIX}{date_string8}_{id}_efficiency-L6_{i}.html"
                        i += 1
                elif effic_opt == 2:
                    filepath = f"{GLOBAL_PLOT_FILE_PREFIX}{date_string8}_{id}_efficiency-L7.html"
                    i = 0
                    while os.path.exists(filepath):
                        filepath = f"{GLOBAL_PLOT_FILE_PREFIX}{date_string8}_{id}_efficiency-L7_{i}.html"
                        i += 1
                elif effic_opt == 3:
                    filepath = f"{GLOBAL_PLOT_FILE_PREFIX}{date_string8}_{id}_efficiency-L8.html"
                    i = 0
                    while os.path.exists(filepath):
                        filepath = f"{GLOBAL_PLOT_FILE_PREFIX}{date_string8}_{id}_efficiency-L8_{i}.html"
                        i += 1
                else:
                    print(
                        "No valid configuration for the Efficiency plot was picked. \n ERROR CODE 51"
                    )
